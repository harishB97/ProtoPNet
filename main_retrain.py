...
for epoch in range(num_train_epochs):
    ...
    if epoch >= push_start and epoch in push_epochs:
        ...
        test_metrics = {'test_'+key : val for key, val in test_metrics.items()}
        metrics_df = metrics_df.append({'ep':str(epoch)+'_'+'after_push', **test_metrics}, ignore_index=True)
        try:
            metrics_df.to_csv(os.path.join(results_dir, 'metrics.csv'))
        except:
            print('Unable to save metrics.csv')
        ...
        if prototype_activation_function != 'linear':
            ...
            for i in range(20):
                ...
                train_metrics = {'train_'+key : val for key, val in train_metrics.items()}
                test_metrics = {'test_'+key : val for key, val in test_metrics.items()}
                metrics_df = metrics_df.append({'ep':str(epoch)+'_'+str(i)+'_'+'after_push', **train_metrics, **test_metrics}, ignore_index=True)
                try:
                    metrics_df.to_csv(os.path.join(results_dir, 'metrics.csv'))
                except:
                    print('Unable to save metrics.csv')
